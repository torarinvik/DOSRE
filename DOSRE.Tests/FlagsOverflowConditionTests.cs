...
public class FlagsOverflowConditionTests
{
    ...
    [Fact]
    public void MakeConditionFromPendingForTest_JoAfterImul_EmitsOverflowCheck()
    {
        var got = DOSRE.Dasm.LEDisassembler.MakeConditionFromPendingForTest(
            jcc: "jo",
            lastWasCmp: false,
        ...
        var expected = "(((int64_t)(int32_t)(eax) * (int64_t)(int32_t)(ecx)) > 0x7fffffffLL) || (((int64_t)(int32_t)(eax) * (int64_t)(int32_t)(ecx)) < (-0x80000000LL))";
        Assert.Equal(expected, got);
    }
}
