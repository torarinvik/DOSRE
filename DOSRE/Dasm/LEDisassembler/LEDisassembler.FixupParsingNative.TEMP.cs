...
namespace DOSRE.Dasm
{
    public static partial class LEDisassembler
    {
        ...
        private static LeFixupRecord? DecipherFixupRecord(byte[] data, int start, int end)
        {
            ...
            // If SourceCount was present, there are subsequent source offsets.
            // But we treat each record as one entry for now, and the loop in ParseAllRecordsForPage
            // should probably handle the extra bytes if p didn't consume them.
            // Actually, if SourceCount is present, Byte 4 was the count, and then there are Count bytes?
            // Spec: "If Bit 5 [SourceType] is 1, a list of source offsets follows... [Byte 4] is Count... 
            // then [target fields]... then [SourceCount - 1] source offsets (each 2 bytes)."
            // Wait, no. The first source offset is ALWAYS at Byte 2.
            // If Count > 1, then the target fields follow the count, and then MORE source offsets follow.
            
            if (rec.SourceCount > 1)
            {
                p += (rec.SourceCount.Value - 1) * 2;
            }
        ...
        private static uint ReadUInt32(byte[] data, int p)
        {
        ...
        private static List<LeFixupRecord> ParseAllRecordsForPage(byte[] stream, int start, int end)
        {
            ...
            while (p < end) {
                ...
                // If this is a multi-fixup record, we should technically expand it.
                // But for now we just return the master record.
